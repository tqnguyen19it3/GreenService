...
class PostNewsController extends Controller
{
    ...
    public function save_posts_news(Request $request){
        ...
        $post_news->post_news_date = $today;
        ...
        if($get_image){
            ...
            $post_news->save();
            Session::put('message', 'Đã thêm bài viết tin tức');
    ...
    public function all_posts_news(){
        $this->AuthLogin();

        $all_posts_news = PostNewsModels::orderBy('post_news_id', 'DESC')->paginate(4);

    	return view('admin.posts.all_news_post')->with(compact('all_posts_news'));
    }
